...
ggplot(cpn,aes(nests))+
  geom_histogram()+
  xlim(0,10000)+ #colonies up to 10000 nests
  theme_few()

ggplot(cpn,aes(nests,chickspernest))+
  geom_point()+
  theme_few()# a single large colony will be excluded too


#eliminate pops with success = 2
...
cpn<-subset(cpn,Ncounts>3) # only colonies with more than 6 counts
...
#the plot shows us that there was site-level slope variability and there was only one site with data previous to 1990. #So, further filters:
...
cpn2<-data.frame(site=cpn$site_id,SP=cpn$common_name,
                 ...
                 chickspernest=cpn$chickspernest) 

counts<-ddply(cpn2, c("SP"), summarise,
              ncounts=length(chickspernest))



shapiro.test(cpn$chickspernest[cpn$chickspernest>0.5]) # normal if eliminate outliers

cpn<-subset(cpn, chickspernest>0.5)
cpn<-subset(cpn,Ncounts>6) # only colonies with more than 6 counts
...
cpnS<-subset(cpn,common_name!="gentoo penguin" | site_name!="Port Charcot") 
cpnS<-subset(cpnS,common_name!="gentoo penguin" | site_name!="Orne Islands") # on this two sites gentoo had less than 6 points


length(unique(cpnS$site_id[cpn$common_name=="adelie penguin"]))
length((cpnS$site_id[cpn$common_name=="adelie penguin"]))
length(unique(cpn$site_id[cpn$common_name=="chinstrap penguin"]))
length((cpnS$site_id[cpn$common_name=="chinstrap penguin"]))
length(unique(cpn$site_id[cpn$common_name=="gentoo penguin"]))
length((cpnS$site_id[cpn$common_name=="gentoo penguin"]))

#site plot

ggplot(cpnS,aes(season_starting,chickspernest,colour=common_name,shape=common_name))+
...
sites<-ddply(cpnS, c("common_name","site_name2","site_id"), summarise,
...
shapiro.test(slst$Slope) # normal

bartlett.test(slst$Slope,slst$locat) # variances are marginally homogeneous between locations

bartlett.test(slst$Slope,slst$common_name)# variances are homogeneous between species
...
# since we checked a priori  distribution and homogeneity, lets only check predictions
posterior_predictive_check(lmsp,iterations=300)
...
fig2<-(ggplot(slst,aes(locat,Slope))+
...
kf.strata<-over(kf.spdf,strata)  # strata names into fishery data
...
kr<-data.frame(krill[1],krill[2],krill[14]) # keep only needed variables for krill data
...
ggarrange(fig3a, fig3b, fig3c, ncol=1)
```
...
shapiro.test(log(data_no_outlier$amlrbiomass)[data_no_outlier$State=="Biomass > mean"])
bartlett.test(log(data_no_outlier$amlrbiomass)[data_no_outlier$State=="Biomass > mean"],data_no_outlier$Strata[data_no_outlier$State=="Biomass > mean"])
...
rm(list= ls()[! (ls() %in% c('cpn','cpn2','kmfcr2',"strata","crs"))])
...
summary(as.factor(cpn2$site))

pcoords<-coordinates(data.frame(cpn2[4:5])) 
p.spdf<-SpatialPointsDataFrame(pcoords,cpn2,proj4string = crs)
...
shapiro.test(pengf$chickspernest[pengf$chickspernest>0.5]) # normal if eliminating outliers

pengf2<-subset(pengf,chickspernest>0.5)


plotdist((pengf2$chickspernest),"norm",para=list(mean=mean((pengf2$chickspernest)), 
                                                 sd=sd((pengf2$chickspernest))))
...
pengf2$HRK2<-factor(pengf2$HRK,levels=c("Biomass > mean, Catch<5%",
                                        ...
                                        "Biomass < mean, Catch>5%"))
bartlett.test(pengf2$chickspernest,pengf2$HRK2) # homogeneity of variances

summary(as.factor(pengf2$HRK2))

lmer2<-lmer(chickspernest~HRK2+(1|site),
            data=pengf2)
anova(lmer2,
      ddf="Satterthwaite",
      type = "III")
...
# what if we only use colonies with over 5 counts?

lmer2b<-lmer(chickspernest~HRK2+(1|site),
            data=subset(pengf2,site=="SHIR"|site=="BART"|site=="ARDL"))
anova(lmer2b,
      ddf="Satterthwaite",
      type = "III")
...
ggplot(subset(pengf2,site=="SHIR"|site=="BART"|site=="ARDL"),aes(HRK2,chickspernest))+
  geom_hline(yintercept=1,
             linetype="dashed",
             linewidth=1)+
  geom_boxplot()+
  ...
  geom_signif(comparisons = list(c("Biomass > mean, Catch<5%",
              ...
              step_increase = 0.075,
              test="t.test", 
              textsize = 3) # still different

# next plots are just to explore a little bit more...


ggplot()+
  geom_point(data=subset(pengf2,site=="SHIR"|site=="BART"|site=="ARDL"),
             aes(x=CatchS,
                 y=chickspernest,
                 colour=KrillC,
                 shape=SP),size=3)+
...
Taking only sites with multiple samples in contrasting krill availability: at ARDL, BART (neighbor areas) increased catch in years with low krill biomass meant decreasing breeding success;  Shireff is the only site in the Drake passage, and trends were inverted; but there was no samples in years with very high catch...
